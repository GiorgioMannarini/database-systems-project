...
class Sort protected (
  ...
  override def execute(): IndexedSeq[HomogeneousColumn] = {
    ...
    val inputVector = inputTable.last.toIndexedSeq.asInstanceOf[IndexedSeq[Boolean]]
    ...
    if (numRows < inputVector.size) inputTable = inputTable.
      map(col => col.toIndexedSeq.zip(inputVector).filter(_._2).map(_._1))
    ...
    reverseCollation.foreach(coll => {
      val column = inputTable(coll.getFieldIndex).toIndexedSeq.zipWithIndex
      ...
      inputTable = inputTable.map(col => indexes.map(col.toIndexedSeq))
    })
